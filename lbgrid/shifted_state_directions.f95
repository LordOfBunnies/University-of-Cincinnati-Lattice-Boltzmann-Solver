subroutine shifted_state_directions(fine)
!
...
IF (cgns_start .OR. mixed_start) THEN
  CALL shifted_state_directions_cgns(fine)
  RETURN
END IF
...
!
            ...
            END IF
          END IF
          if (a == 267 .and. b == 21 .and. c == 176) then
            write(*,*) 'state secrets',state(a+cx(i),b+cy(i),c+cz(i),1),i,state(a,b,c,1:dir),self
          end if
...
!          end if
          if (state(a,b,c,i) == -1001) state(a,b,c,i) = lvl
        END DO x_loop
      END DO y_loop
    END DO z_loop
  END DO dir_loop
