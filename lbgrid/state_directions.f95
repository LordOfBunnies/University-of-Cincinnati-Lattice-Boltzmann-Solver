subroutine state_directions(fine)
!
...
!
          ELSE
            IF (state(a+cx(i),b+cy(i),c+cz(i),1) == -1001 .and. .not. shifted) THEN
              call inter_bb_setup(state(a,b,c,i),state(a,b,c,1),i,a,b,c,lvl)
            else if (shifted  .and. a-cx(i) >= sax%lo(1)-nghosts_mid .and. &
              b-cy(i) >= sax%lo(2)-nghosts_mid .and. c-cz(i) >= sax%lo(3)-nghosts_mid .and. &
              a-cx(i) <= sax%hi(1)+nghosts_mid .and. b-cy(i) <= sax%hi(2)+nghosts_mid .and. &
              c-cz(i) <= sax%hi(3)+nghosts_mid ) then
