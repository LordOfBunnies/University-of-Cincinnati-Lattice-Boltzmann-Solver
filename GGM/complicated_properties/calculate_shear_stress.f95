SUBROUTINE calculate_shear_stress(lvl,tux_lo,tux_hi,g,choice,mfi)
!
! This calculate the local shear stress to populate the Q matrix
!
! Calls:
...
IMPLICIT NONE
INTEGER :: a,b,c
integer,intent(in) :: tux_lo(3),tux_hi(3),lvl,g,choice
integer :: substate(5),sub_u(5),sub_v(5),sub_w(5)
...
!
! Information held in sub-variables
! node(1),node+1(2),node+2(3),node-1(4),node-2(5)
!
!
!
...
!            CALL x_dir_derivative(a,tau_dvdx,in_values)
!
...
!            CALL y_dir_derivative(a,tau_dudy,in_values)
!
...
!            CALL x_dir_derivative(a,tau_dwdx,in_values)
!
!          ELSE IF (deriv_form(1,2,a)) THEN
!            in_values(3) = w_vel(a)
!            in_values(2) = w_vel(link(5,a))
...
!            CALL z_dir_derivative(a,tau_dudz,in_values)
!
...
!            CALL y_dir_derivative(a,tau_dwdy,in_values)
!
!          ELSE IF (deriv_form(2,2,a)) THEN
!            in_values(3) = w_vel(a)
!            in_values(2) = w_vel(link(6,a))
...
!            CALL z_dir_derivative(a,tau_dvdz,in_values)
!
!          ELSE IF (deriv_form(3,2,a)) THEN
!            in_values(3) = v_vel(a)
!            in_values(2) = v_vel(link(7,a))
...
!          END IF
!
!          q(a) = SQRT((tau_dudy+tau_dvdx)**2.0D0 + (tau_dudz+tau_dwdx)**2.0D0 + &
!            (tau_dvdz+tau_dwdy)**2.0D0 )
!
!        END DO
!
!      END IF
